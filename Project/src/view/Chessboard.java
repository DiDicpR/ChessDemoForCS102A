...
public class Chessboard extends JComponent {
    ...
    public int count=0 ;
    public List<List<String>> ArchiveGame = new ArrayList<>();
    ...
    public Chessboard(int width, int height) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 8; j++) {
                if(i==0)
                    initPawnOnBoard(5*i+1,j,ChessColor.BLACK);
                else
                    initPawnOnBoard(5*i+1,j,ChessColor.WHITE);
            }
        }
    }
    ...
    public void swapChessComponents(ChessComponent chess1, ChessComponent chess2) {
        if(chess1.getChessName()=='p'&&chess1.getChessboardPoint().getX()==1){
        ...
        if(chess1.getChessName()=='P'&&chess1.getChessboardPoint().getX()==6){
            remove(chess1);
